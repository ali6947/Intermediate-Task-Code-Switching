...
def train(args, train_dataset, model, tokenizer):
    ...
    logger.info("  Num examples = %d", 2*len(train_dataset))
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.model_type != "distilbert":
                inputs["token_type_ids"] = (
                    torch.cat((batch[2],batch[6]),0) if args.model_type in ["bert"] else None
                )  # XLM and DistilBERT don't use segment_ids
            # print(type(inputs),end='***$$\n')
            # for i1 in inputs.keys():
            #     print(type(inputs[i1]))
            #     print(i1+' '+str(inputs[i1].shape))
            outputs = model(**inputs)
            loss = outputs[0]  # model outputs are always tuple in transformers (see doc)
            nli_pred = outputs[1]
            _,nli_labels=torch.max(nli_pred,-1)
            # print(nli_labels.shape)
            # print(batch[3].shape)
            nrnli+=torch.eq(nli_labels,torch.cat(\
                    (batch[3],batch[7]),0)).sum().item()
            drnli+=float(batch[3].shape[0])+float(batch[7].shape[0])
            lossnli+=loss.item()
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step%args.logging_steps==0:
                    ...
                    if val_acc>best_val_acc:
                        if args.save_model:
                            logger.info('new acc at %s, saving model',str(val_acc))
                            ...
                            logger.info('new acc at %s, no model saved',str(val_acc))
                            best_val_acc=val_acc
                    else:
                        logger.info('acc still at %s',str(best_val_acc))
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        logger.info("  Num examples = %d", 2*len(eval_dataset))
        logger.info("  Batch size = %d", 2*args.eval_batch_size)
        eval_loss = 0.0
        nb_eval_steps = 0
        preds = None
        out_label_ids = None
        ...
        valacc=float(np.sum(np.equal(preds,out_label_ids)))/float(preds.shape[0])
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        logger.info("Creating features from dataset file at %s", args.data_dir1)
        # print("hey")
        label_list = processor.get_labels()
        examples1 = (
            processor.get_test_examples(args.data_dir1) if evaluate else processor.get_train_examples(args.data_dir1)
        )
        # print(examples[0])
    ...
    logger.info("Creating romanised features from dataset file at %s", args.data_dir2)
        # print("hey")
    # label_list = processor.get_labels()
    examples2 = (
        processor.get_test_examples(args.data_dir2) if evaluate else processor.get_train_examples(args.data_dir2)
    )
    # print(examples[0])
    # print(len(examples[0].text_a.split(' '))+len(examples[0].text_b.split(' ')))
    ...
    dataset = TensorDataset(all_input_ids, all_attention_mask, all_token_type_ids, all_labels,\
        all_input_ids2, all_attention_mask2, all_token_type_ids2, all_labels2)
    return dataset
...
def main():
    ...
    parser.add_argument("--save_model", action="store_true", help="Saves model with current best validation accuracy.")
    parser.add_argument(
    ...
    if args.do_train:
        ...
        # model.save_pretrained('./pretrained_on_bilingual_xnli/final_model')
        # tokenizer.save_pretrained('./pretrained_on_bilingual_xnli/final_model')
        # torch.save(args, os.path.join('./pretrained_on_bilingual_xnli/final_model', "training_args.bin"))
    # Evaluation
    results = {}
    if args.do_eval and args.local_rank in [-1, 0]:
        pred_labels,_ = evaluate(args, model, tokenizer)# prefix=global_step)

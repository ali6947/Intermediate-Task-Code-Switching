...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            nrnli+=torch.eq(nli_labels,batch[3]).sum().item()
            drnli+=float(batch[3].shape[0])
            lossnli+=loss.item()
            iters+=1
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    if args.model_type=='bert':
        ...
        all_token_type_ids = torch.tensor([tokenizer.create_token_type_ids_from_sequences(f.attention_mask) for f in features], dtype=torch.long)
...
def main():
    ...
    if args.do_train:
        ...
        # model.save_pretrained('./azure_ml')
    # Evaluation
    results = {}
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        # results.update(result)
        # with open('{}/goog_test_predictions'.format(args.output_dir)+'seed'+str(args.seed)+'ep'+str(int(args.num_train_epochs))+'.txt', 'w') as f:
